...
class LikeCommentUseCase @Inject constructor(
    ...
    operator fun invoke(jwt: String?, commentId: String): Flow<Response<CommentDTO>> = flow {
        try {
            emit(Response.Loading<CommentDTO>())
            val data = repository.likeComment(authHeader = "Bearer $jwt", commentId = commentId)
